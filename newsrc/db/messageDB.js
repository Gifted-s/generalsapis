...
function makeMessageDB({
  ...
  async function editMessage({
    ...
    const db = await makeDB();
    const query = await db.collection('messages').updateOne({
      id
    }, {
      $set: { ...messageChanges
      }
  ...
  async function insertMessage({ ...messageInfo
    ...
    const query = await db.collection('messages').insertOne({ ...messageInfo,
  ...
  async function deleteMessage({
    ...
    const db = await makeDB();
    const query = await db.collection('messages').deleteOne({
      id
    });
